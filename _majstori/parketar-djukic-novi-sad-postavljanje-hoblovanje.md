---
layout: majstor_profil
title: Parketar Đukić Novi Sad – Podovi Koji Traju Decenijama
description: Parketar Đukić – postavljanje, hoblovanje, lakiranje, laminat, sokle.
  15+ godina iskustva, pedantno, u roku. Novi Sad i okolina. u Novi Sad, Veternik,
  Futog, Petrovaradin, Sremska Kamenica, Beočin, Rumenka, Kać, Temerin, Bački Petrovac.
ime: Parketar Đukić Novi Sad – Podovi Koji Traju Decenijama
kategorija:
- Postavljanje Parketa
- Hoblovanje Parketa
- Lakiranje
- Laminat
- Brodski Pod
mesto: Novi Sad, Veternik, Futog, Petrovaradin, Sremska Kamenica, Beočin, Rumenka,
  Kać, Temerin, Bački Petrovac
address: Đorđa Rajkovića 10, Novi Sad 21000
kontakt_telefon: "+381640567420"
email: parketardjukic88@gmail.com
googlemaps: ''
website:
  url: https://www.parketardjukic.rs/
  label: Link do sajta
slug: parketar-djukic-novi-sad-postavljanje-hoblovanje
opening_hours:
thumbnail: images/parketar-djukic-novi-sad-postavljanje-hoblovanje.webp
hero_image: images/parketar-djukic-novi-sad-postavljanje-hoblovanje.webp
discover_title: Parketar Đukić Novi Sad – Podovi Koji Traju Decenijama
author: Petar Škrbić
author_slug: petar-skrbic
author_bio: Petar je osnivač platforme Ej Majstore! online adresara zanatskih i uslužnih
  delatnosti u Srbiji. Platforma prikuplja i objavljuje osnovne informacije o izvođačima
  dostupne javnosti ili dostavljene od strane samih firmi. Lični osvrt autora ne predstavlja
  preporuku niti garanciju kvaliteta.
date: '2025-11-03'
last_modified: '2025-11-03'
sections:
- type: p
  content: U Novom Sadu, gde <em>stari parket škripi, a novi čeka na majstora</em>
    – <strong>Parketar Đukić</strong> donosi 15+ godina iskustva. Od hoblovanja do
    mozaika – sve <em>pedantno, u roku, po povoljnoj ceni</em>.
- type: p
  content: Baziran u <a href="https://maps.app.goo.gl/PAavJ7jDdv4fP5va9" target="_blank"
    rel="noopener nofollow"><strong>Đorđa Rajkovića 10</strong></a>, pokriva <strong>ceo
    Novi Sad i okolinu</strong>. Pogledajte radove na <a href="https://www.parketardjukic.rs/"
    target="_blank" rel="noopener nofollow"><strong>parketardjukic.rs</strong></a>.
- type: h2
  content: Ko je Parketar Đukić – Tradicija u Drvetu
- type: p
  content: 'Kao firma funkcioniše od 2015. Specijalisti za tvrde podne obloge – od
    klasičnog parketa do dekorativnih mozaika. Lično još nisam imao priliku da sarađujem
    sa ovim izvođačem, ali na osnovu razgovora sa ljudima koji jesu može se zaključiti
    da je trenutno izvođač #1 što se tiče postavljanja drvenih podova u Novom Sadu.'
- type: p
  content: "Najpovoljniji način kontakta je na sledeće načine:<br>\U0001F4DE <strong>Telefon:</strong>
    <a href=\"tel:+381640567420\">+381 64 056 74 20</a><br>\U0001F4E7 <strong>Email:</strong>
    <a href=\"mailto:parketardjukic88@gmail.com\">parketardjukic88@gmail.com</a><br>\U0001F310
    <strong>Veb-sajt:</strong> <a href=\"https://www.parketardjukic.rs/\" target=\"_blank\"
    rel=\"noopener nofollow\">parketardjukic.rs</a><br>\U0001F4CD <strong>Lokacija:</strong>
    <a href=\"https://maps.app.goo.gl/PAavJ7jDdv4fP5va9\" target=\"_blank\" rel=\"noopener
    nofollow\">Đorđa Rajkovića 10, Novi Sad</a>"
- type: h2
  content: Sve za Savršen Pod
- type: p
  content: 'Kompletna usluga za drvene i tvrde podove:'
- type: h3
  content: 1. Postavljanje Parketa
- type: p
  content: Klasični, višeslojni, brodski pod – lepljenje ili na klik.
- type: h3
  content: 2. Hoblovanje i Lakiranje
- type: p
  content: Stari parket kao nov – tri sloja laka, poliranje.
- type: h3
  content: 3. Postavljanje Laminata
- type: p
  content: Brzo, povoljno, sa podlogom i završnim lajsnama.
- type: h3
  content: 4. Sokle i Lajsne
- type: p
  content: PVC, drvene, visoke – savršen završetak.
- type: h3
  content: 5. Dekorativni Mozaici
- type: p
  content: Umetnički podovi – po meri, po želji.
- type: h2
  content: Gde Radi Parketar Đukić – Novi Sad i Vojvodina
- type: p
  content: 'Baziran u <strong>Novom Sadu</strong>, dolazi u:'
- type: ul
  items:
  - "<strong>Veternik, Futog</strong> – hoblovanje starog parketa"
  - "<strong>Petrovaradin, Liman</strong> – postavljanje laminata"
  - "<strong>Sremska Kamenica, Telep</strong> – brodski pod"
  - "<strong>Rumenka, Kać</strong> – sokle i lajsne"
  - "<strong>Beočin, Temerin</strong> – kompletna renovacija"
- type: h2
  content: Kako Funkcioniše Saradnja sa Parketarom Đukićem
- type: p
  content: "<strong>1. Poziv</strong> – dogovor i procena<br><strong>2. Merenje</strong>
    – besplatno, tačno<br><strong>3. Priprema</strong> – košuljica, vlažnost<br><strong>4.
    Rad</strong> – postavljanje, hoblovanje, lakiranje<br><strong>5. Završetak</strong>
    – čišćenje, predaja"
- type: h2
  content: Zašto Parketar Đukić – Ključne Prednosti
- type: ul
  items:
  - 15+ godina iskustva – poznavanje svakog detalja
  - U roku – nikad kašnjenja
  - Povoljne cene – kvalitet bez preplate
  - Kompletna usluga – od merenja do čišćenja
  - Zadovoljni klijenti – najbolja reklama
  - Saveti za održavanje – pod traje duže
- type: blockquote
  content: <strong><a href="https://share.google/9ojzi0VxQY2XSbLku" target="_blank"
    rel="noopener nofollow">Aleksandar Jovanovic</a>:</strong> Перфектно постављен
    паркет. Хоблован и лакиран. А у другом делу стана хоблован и лакиран стари паркет.
    Перфекција. Педантно, брзо, одговорно.
- type: blockquote
  content: <strong><a href="https://share.google/H9h7wRnya74IiTzc7" target="_blank"
    rel="noopener nofollow">Predrag Ubovic</a>:</strong> Svaka čast, Hoblovanje parketa
    su završili veoma brzo. Prelep je sad parket. Preporučujem ih svakom kome trebaju
    pravi majstori za parket. Uredno sve završeno veliki pozdrav
- type: blockquote
  content: <strong><a href="https://share.google/iktXZTVufEFKtXsCX" target="_blank"
    rel="noopener nofollow">Miroslav Boskovic</a>:</strong> Profesinalno, kvalitetno
    i pre roka. Sve preporuke i misljenje da je bolje stari parket izoblovati preko
    Đukića nego postaviti novi. @apartman_innocence
- type: p
  content: 'Ako vaš <strong>pod čeka na novi život</strong> – <strong>Parketar Đukić</strong>
    je pravi izbor. Posetite <a href="https://www.parketardjukic.rs/" target="_blank"
    rel="noopener nofollow"><strong>parketardjukic.rs</strong></a> ili pozovite: <a
    href="tel:+381640567420"><strong>+381 64 056 74 20</strong></a>.'
services:
- title: Postavljanje Parketa
  description: Klasični, višeslojni, brodski pod.
- title: Hoblovanje i Lakiranje
  description: Stari parket kao nov – tri sloja laka.
- title: Postavljanje Laminata
  description: Brzo, povoljno, sa podlogom.
- title: Sokle i Lajsne
  description: PVC, drvene, visoke.
- title: Dekorativni Mozaici
  description: Umetnički podovi po meri.
faq:
- question: Može li parket na podno grejanje?
  answer: Da – samo višeslojni, vlažnost <2%.
- question: Koliko traje hoblovanje?
  answer: 1–2 dana za stan 60m².
- question: Koji lepak koristite?
  answer: Mapei, Sipol, Loba – dvokomponentni.
- question: Imate li garanciju?
  answer: Da – na rad i materijal.
- question: Radite li vikendom?
  answer: Da, po dogovoru.
- question: Kako da vas kontaktiram?
  answer: Pozovite +381 64 056 74 20 ili email parketardjukic88@gmail.com
tips:
- Višeslojni parket za podno grejanje – nikad jednoslojni
- Vlažnost košuljice ispod 2% – obavezno pre ugradnje
- Dvokomponentni lepak – jači, dugotrajniji
- Brisanje mlakom vodom – bez hemije na laku
- Provetravanje posle brisanja – sprečava bubrenje
- Hoblovanje starog parketa – bolje od novog
tips_title: Obezbedite da Vaš Pod Traje Generacijama
permalink: "/izvodjaci/novi-sad/parketar-djukic-novi-sad-postavljanje-hoblovanje/"
disable_seo: true
---
